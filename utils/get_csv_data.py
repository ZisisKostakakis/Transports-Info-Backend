...
import pandas as pd
import csv
from utils.common_vars import TRAIN, BUS, FLIGHTS, DATA_DIRECTORY
...
def get_csv_data(transportation_type, verbose):
    try:
        ...
        df = pd.DataFrame()
        df = pd.read_csv(f'{DATA_DIRECTORY}{transportation_type}.csv', encoding='utf-8')
        if verbose:
            print(df)
            return True, df
...
def main():
    ...
    if success:        
        ...
        if verbose:
            msg='Failed to retrieve the {}.csv file'.format(transportation_type)
            print(msg)
    return 
